...
export class PayrollEngine {
  ...
  static calculateStaffRemuneration(
    ...
    options: { skipStatutory?: boolean; lwpDays?: number; totalDays?: number } = {}
  ): SalaryBreakdown {
    ...
    if (options.lwpDays && options.totalDays && options.lwpDays > 0) {
      lwp = this.roundINR((grossRemuneration / options.totalDays) * options.lwpDays);
    }
