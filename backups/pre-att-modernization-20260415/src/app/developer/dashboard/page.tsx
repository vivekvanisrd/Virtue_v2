...
import { StaffTransferModal } from "@/components/staff/StaffTransferModal";
...
export default function DeveloperDashboardPage() {
  ...
  const [showEditBranch, setShowEditBranch] = useState<any>(null);
  const [showTransferStaff, setShowTransferStaff] = useState<any>(null);
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] text-slate-900 font-sans selection:bg-blue-100 p-4 lg:p-6">
                     ...
                     {globalData?.schools?.map((s: any) => (
                       <div key={s.id} className="p-6 bg-slate-50 border border-slate-200/60 rounded-3xl hover:border-blue-400/40 hover:bg-white transition-all cursor-pointer group shadow-sm hover:shadow-md">
                         ...
                         <div className="mt-4 space-y-2 border-t border-slate-200/60 pt-4">
                            ...
                            {(globalData?.branches || []).filter((b: any) => b.schoolId === s.id).map((b: any) => (
                                <div key={b.id} className="flex items-center justify-between p-2.5 bg-white border border-slate-200/60 rounded-xl hover:border-blue-300 transition-colors group/branch">
                                   <div className="flex flex-col">
                                      <span className="text-[10px] font-bold text-slate-700">{b.name}</span>
                                      <span className="text-[9px] font-mono text-slate-400">{b.id} ({b.code})</span>
                                   </div>
                                   <button 
                                     onClick={(e) => { e.stopPropagation(); setShowEditBranch(b); }} 
                                     className="p-1.5 opacity-0 group-hover/branch:opacity-100 hover:bg-blue-50 rounded-lg transition-all"
                                   >
                                      <Settings className="w-3.5 h-3.5 text-slate-400 hover:text-blue-600" />
                                   </button>
                                </div>
                            ))}
                         </div>
                            ...
                            <tr key={u.id} className="hover:bg-slate-50 transition-colors group">
                              ...
                              <td className="p-4">
                               ...
                               <td className="p-4 text-right">
                                 ...
                                 <button 
                                   onClick={() => setShowTransferStaff(u)}
                                   className="p-2 hover:bg-indigo-50 text-slate-400 hover:text-indigo-600 rounded-lg transition-all opacity-0 group-hover:opacity-100"
      ...
      <AnimatePresence>
        ...
        {showEditBranch && (
          <BranchModal 
            branch={showEditBranch}
            onClose={() => setShowEditBranch(null)}
            onSuccess={() => { setShowEditBranch(null); fetchGlobalData(); refreshData(); }}
          />
        )}
        {showTransferStaff && (
          <StaffTransferModal
            staff={showTransferStaff}
            onClose={() => setShowTransferStaff(null)}
            onSuccess={() => { setShowTransferStaff(null); fetchGlobalData(); }}
          />
        )}
      </AnimatePresence>
...
function BranchModal({ branch, onClose, onSuccess }: any) {
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState({
    name: branch.name,
    code: branch.code || "",
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-8">
           <h3 className="text-xl font-black text-slate-900 tracking-tighter uppercase">Edit Branch Protocol</h3>
           <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-full"><X className="w-5 h-5 text-slate-400" /></button>
        </div>
