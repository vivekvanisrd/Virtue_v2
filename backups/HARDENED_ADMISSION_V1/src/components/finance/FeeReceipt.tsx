...
export function FeeReceipt({ student, receipt, schoolInfo }: FeeReceiptProps) {
  const defaultSchool = {
    name: "PaVa-EDUX Academy",
    address: "Institutional Square, Jubilee Hills, Hyderabad - 500033",
    phone: "+91 91234 56789",
    email: "accounts@pava-edux.edu.in",
  };
  ...
  const handlePrint = () => {
    ...
    const content = `
      ...
      <html>
        <head>
          <title>PaVa-EDUX Receipt - ${receipt.receiptNumber}</title>
          ${styleHtml}
          <style>
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-slate-500">
                #{receipt.admission?.admissionNumber || student.admissionNumber || student.studentCode || "N/A"} • {student.academic?.class?.name}-{student.academic?.section?.name}
              </p>
            </div>
          </div>
        </div>
