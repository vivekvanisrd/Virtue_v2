 ...
 */
export async function importStaffCSV(records: any[], targetSchoolId: string, targetBranchId: string): Promise<ImportResult> {
  ...
  if (identity.schoolId !== targetSchoolId && !identity.isGlobalDev) {
    throw new Error("SECURITY_VIOLATION: Attempted to import records into an unauthorized institution.");
  }
