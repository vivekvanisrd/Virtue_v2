...
export async function POST(req: NextRequest) {
  try {
    ...
    if (event === "order.paid" || event === "payment.captured") {
      ...
      const totalPaid = Number(payment.amount || 0) / 100;
      let batchData: Array<{studentId: string, baseAmount: number, lateFee: number, terms: string[]}> = [];
      
      if (notes.batch) {
        batchData = notes.batch.split("|").map((s: string) => {
          const parts = s.split(":");
          return { studentId: parts[0], baseAmount: Number(parts[1]), lateFee: 0, terms: parts[2].split(",") };
        });
      } else if (notes.studentId) {
        const baseAmount = Number(notes.baseAmount || totalPaid / 1.0177);
        const termsRaw = notes.terms || notes.selectedTerms || "";
        batchData = [{ studentId: notes.studentId, baseAmount, lateFee: Number(notes.lateFee || 0), terms: termsRaw.split(",").filter(Boolean) }];
      } else {
      ...
      if (batchData.length > 0) {
        const totalConvenienceFee = Number(notes.gatewayFee)
          ...
          : totalPaid - batchData.reduce((sum, b) => sum + b.baseAmount + b.lateFee, 0);
        const perStudentConvenienceFee = totalConvenienceFee / batchData.length;
        ...
        for (const batchItem of batchData) {
          await processCollectionFlow({
            ...
            baseAmount: batchItem.baseAmount,
            lateFee: batchItem.lateFee,
    ...
    if (event === "payment_link.paid") {
      ...
      if (notes.type === "FEE_COLLECTION" || notes.type === "FEE_COLLECTION_V2" || notes.type === "FEE_COLLECTION_V2_TAXED") {
        ...
        if (notes.batch) {
          ...
          const baseAmount = Number(notes.baseAmount || totalPaid / 1.0177);
          const termsRaw = notes.terms || notes.selectedTerms || "";
          batchData = [{ studentId: notes.studentId, baseAmount, lateFee: 0, terms: termsRaw.split(",").filter(Boolean) }];
        }
...
async function processCollectionFlow(data: {
  ...
  // 1. Idempotency — never double-record per student per payment
  ...
  if (existing) {
    console.log(`[RAZORPAY_WEBHOOK] ℹ️ ${data.referenceId} already settled for student ${data.studentId}. Skipping.`);
    return;
  }
  ...
  try {
    await prisma.$transaction(async (tx: any) => {
      ...
      // Promote student if they are currently Provisional
      if (student.status === "Provisional") {
          const promotion = await promoteStudentAction(data.studentId, tx);
          if (!promotion.success) {
              throw new Error(`Auto-Promotion Failed during settlement: ${promotion.error}`);
          }
          console.log(`[RAZORPAY_WEBHOOK] ✅ Student ${data.studentId} promoted to ACTIVE.`);
      }
