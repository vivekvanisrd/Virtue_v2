...
export function StudentFinancialHub({ studentId }: StudentFinancialHubProps) {
  ...
  return (
    <div className="space-y-8 w-full max-w-[1400px] mx-auto p-4 md:p-8 animate-in fade-in slide-in-from-bottom-5 duration-700">
      {/* ─── EXECUTIVE FINANCIAL VITAL SIGNS (NON-REDUNDANT & INTERACTIVE) ─── */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {/* Card 1: Net Outstanding */}
        <div className="bg-slate-900 text-white p-6 rounded-3xl shadow-xl relative overflow-hidden flex flex-col justify-between border border-slate-800 group">
          <div className="flex items-center justify-between mb-4">
            <span className="text-[9px] font-black uppercase tracking-[0.25em] text-slate-400">Net Outstanding</span>
            <div className="w-8 h-8 bg-white/10 rounded-xl flex items-center justify-center border border-white/10">
              <Wallet className="w-4 h-4 text-emerald-400" />
            </div>
          </div>
          <div className="space-y-1">
            <h3 className="text-3xl lg:text-4xl font-black italic tracking-tighter tabular-nums">
              {formatCurrency(outstanding)}
            </h3>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
              {outstanding === 0 ? "✓ Fully Cleared" : "Outstanding Dues Unsettled"}
            </p>
          </div>
          <div className="mt-5 pt-3 border-t border-white/10 flex items-center justify-between">
            {firstUnpaid ? (
              <button 
                ...
                title="Click to settle upcoming installment"
              >
                Settle {firstUnpaid.label} <ArrowRight className="w-3 h-3" />
              </button>
            ) : (
              <span className="text-[9px] font-black uppercase tracking-widest text-emerald-400">All Settled</span>
            )}
            <button 
              onClick={() => setView('audit')} 
              className="text-[9px] font-black uppercase tracking-widest text-white/50 hover:text-white flex items-center gap-1"
              title="Click to open Audit Ledger Registry"
            >
      ...
      {/* 🧾 ITEMIZED BILL DETAILS MODAL */}
      {isBillDetailsOpen && (
        <div className="fixed inset-0 z-[120] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-3xl rounded-[2.5rem] border border-slate-100 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="px-8 py-6 bg-slate-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-black tracking-tight">Itemized Bill Breakdown</h3>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Master Fee Schedule & Ancillary Charges</p>
                </div>
              </div>
              <button 
            ...
            <div className="p-8 space-y-6 max-h-[60vh] overflow-y-auto">
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                      ...
                      <th className="p-4 text-right">Concession</th>
                      <th className="p-4 text-right">Net Payable</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 text-xs font-bold">
                    {(studentData?.financial?.components || []).map((comp: any) => {
                      const base = Number(comp.baseAmount) || 0;
                      const disc = Number(comp.discountAmount) || 0;
                      const net = Number(comp.netAmount) || base - disc;
                      return (
                        <tr key={comp.id} className="hover:bg-slate-50/80 transition-colors">
                          <td className="p-4 text-slate-900">
                            {comp.masterComponent?.name || "Tuition Base Fee"}
                          </td>
                          <td className="p-4 text-right text-slate-900">{formatCurrency(base)}</td>
                          <td className="p-4 text-right text-emerald-600">
                            {disc > 0 ? `-${formatCurrency(disc)}` : "—"}
                          </td>
                          <td className="p-4 text-right font-black text-slate-900">{formatCurrency(net)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
              ...
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 space-y-2">
                <div className="flex justify-between text-xs font-semibold text-slate-600">
                  <span>Gross Actual Fee Schedule</span>
                  <span className="font-bold text-slate-900">{formatCurrency(grandTotalFee + (Number(studentData.financial?.totalDiscount) || 0))}</span>
                </div>
                <div className="flex justify-between text-xs font-semibold text-emerald-600">
                  <span>Approved Policy Concession</span>
                  <span className="font-bold">-{formatCurrency(Number(studentData.financial?.totalDiscount) || 0)}</span>
                </div>
                <div className="flex justify-between text-sm font-black text-slate-900 pt-2 border-t border-slate-200">
                  <span>Net Annual Payable</span>
                  <span>{formatCurrency(grandTotalFee)}</span>
                </div>
              </div>
            </div>
      ...
      {/* 💳 TRANSACTION RECEIPTS LOG MODAL */}
      {isTransactionHistoryOpen && (
        <div className="fixed inset-0 z-[120] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-4xl rounded-[2.5rem] border border-slate-100 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="px-8 py-6 bg-slate-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center border border-white/20">
                ...
                <div>
                  <h3 className="text-base font-black tracking-tight">Transaction Receipts & Collection Log</h3>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Audited POS Payment History</p>
                </div>
              </div>
              <button 
              ...
              {(studentData?.collections || []).length > 0 ? (
                <div className="border border-slate-200 rounded-2xl overflow-hidden">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100 text-xs font-bold">
                      {studentData.collections.map((coll: any) => (
                        <tr key={coll.id} className="hover:bg-slate-50/80 transition-colors">
                          <td className="p-4 text-slate-900 font-black">{coll.receiptNo || coll.id.substring(0, 8)}</td>
                          <td className="p-4 text-slate-600">{new Date(coll.createdAt || coll.paymentDate).toLocaleDateString()}</td>
                          <td className="p-4">
                            <span className="px-2.5 py-1 bg-slate-100 text-slate-700 rounded-lg text-[10px] font-black uppercase">
                              {coll.paymentMode || "CASH"}
                            </span>
                          </td>
                          <td className="p-4 text-right font-black text-emerald-600">{formatCurrency(coll.amountPaid || coll.amount)}</td>
                          <td className="p-4 text-center">
                            <button 
                              onClick={() => setSelectedReceipt(coll)}
                              className="px-3 py-1.5 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all flex items-center gap-1 mx-auto"
                            >
                ...
                <div className="text-center py-12 text-slate-400 font-bold uppercase tracking-widest text-xs border border-dashed border-slate-200 rounded-2xl bg-slate-50/50">
                  No payment receipts recorded yet for this student.
                </div>
              )}
            </div>
            ...
            <div className="px-8 py-5 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
              <span className="text-xs font-bold text-slate-500">
                Total Settled: <span className="text-emerald-600 font-black">{formatCurrency(totalCollected)}</span>
              </span>
              <button 
                onClick={() => setIsTransactionHistoryOpen(false)}
                className="px-6 py-2.5 bg-slate-900 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-800 transition-all"
      ...
      {/* 📄 PRINTABLE RECEIPT VIEWER OVERLAY */}
      {selectedReceipt && (
        <div className="fixed inset-0 z-[150] bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white w-full max-w-3xl rounded-[2.5rem] border border-slate-100 shadow-2xl p-8 space-y-6 relative my-8">
            <button 
              onClick={() => setSelectedReceipt(null)}
              className="absolute top-6 right-6 p-2 text-slate-400 hover:text-slate-950 hover:bg-slate-100 rounded-xl transition-all z-20"
            >
              <X className="w-6 h-6" />
            </button>

            <FeeReceipt student={studentData} receipt={selectedReceipt} />
          </div>
        </div>
      )}
    </div>
