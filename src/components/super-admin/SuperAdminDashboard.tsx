      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-slate-500 font-bold mt-2">Managing {schools.length} unique school instances in the ecosystem.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            <motion.div
              ...
              key={i}
              className={cn("p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col justify-between h-32", stat.bg)}
            >
        ...
        <div className="grid grid-cols-1 gap-4">
          {isLoading ? (
            <div className="py-20 flex flex-col items-center justify-center gap-4">
              ...
              <p className="text-slate-400 font-bold italic tracking-widest">Accessing Registry...</p>
            </div>
            ...
            filteredSchools.map((school, i) => (
              <motion.div
                ...
                key={school.id}
                className="group bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:border-primary/20 transition-all flex flex-col md:flex-row md:items-center justify-between gap-6"
              >
                <div className="flex items-center gap-6">
                  <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center group-hover:bg-primary/5 transition-colors overflow-hidden">
                    {school.logo ? (
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="text-xl font-black text-slate-900 italic">{school.name}</h3>
                      <span className="px-2 py-0.5 bg-slate-100 text-[10px] font-black text-slate-500 rounded-md uppercase">
                        {school.code}
                      </span>
                    </div>
                ...
                <div className="flex items-center gap-3">
                  <div className="hidden lg:flex items-center gap-3 mr-4">
                    <div className="text-right">
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Active Since</p>
                      <p className="text-sm font-black text-slate-700 italic">
                        {new Date(school.createdAt).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                  <button className="px-4 py-2 border-2 border-slate-100 hover:border-primary/20 hover:bg-primary/5 rounded-xl font-bold text-slate-600 hover:text-primary transition-all flex items-center gap-2">
                    Manage
            ...
            <div className="py-20 text-center space-y-4 bg-slate-50/50 rounded-[40px] border-2 border-dashed border-slate-200">
              ...
              <p className="text-slate-400 font-bold italic tracking-widest uppercase text-sm">No schools found matching search</p>
            </div>
          )}
        </div>
      </div>
            ...
            >
              <div className="p-8 lg:p-12">
                <div className="flex items-center justify-between mb-8">
                  <div>
                    <h2 className="text-3xl font-black text-slate-800 italic">Provision New School</h2>
                    <p className="text-slate-500 font-bold">Deploy a fresh school instance and first branch node.</p>
                  </div>
                  <button 
                    onClick={() => setIsModalOpen(false)}
