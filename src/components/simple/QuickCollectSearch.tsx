...
export function QuickCollectSearch({ autoFocus }: { autoFocus?: boolean }) {
      ...
      {results.length > 0 && (
        <ul style={{ marginTop: 14, listStyle: "none", padding: 0, display: "grid", gap: 8 }}>
          {results.map((r) => (
            <li key={r.id}>
              <button
                onClick={() => router.push(`/simple/collect?studentId=${r.id}`)}
                style={{
                  width: "100%",
                  textAlign: "left",
                  padding: "14px 16px",
              ...
              >
                <span>
                  ...
                  {r.admissionNumber && <span style={{ color: "#6b7280", marginLeft: 8 }}>#{r.admissionNumber}</span>}
                </span>
                <span style={{ color: "#6b7280", fontSize: 14 }}>{[r.className, r.branchName].filter(Boolean).join(" · ")}</span>
              </button>
            </li>
          ))}
        </ul>
