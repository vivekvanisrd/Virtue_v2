...
export function Header({ 
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      setErrorText(res.error || "Failed to update password.");
    }
    setIsLoading(false);
  };
  ...
  return (
    ...
    {isChangePasswordOpen && (
      <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm text-left">
        <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl border border-slate-100 space-y-6 animate-in zoom-in-95 duration-200">
          <div className="flex items-center justify-between pb-2 border-b border-slate-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mt-0.5">Secure Update Protocol</p>
              </div>
            </div>
            <button 
              onClick={() => {
          ...
          <form onSubmit={handlePasswordChange} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-4 py-3 bg-slate-50 border-2 border-slate-100 rounded-xl outline-none focus:border-indigo-500 focus:bg-white transition-all text-sm font-semibold text-slate-800"
                />
                <button
                  type="button"
            ...
            <button
              ...
              className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white rounded-xl font-black text-sm flex items-center justify-center gap-2 transition-all shadow-lg shadow-indigo-100 active:scale-[0.98]"
            >
              {isLoading ? (
                <>
