  ...
  return (
    <div className="relative">
      <button 
        ...
        className="w-10 h-10 lg:w-12 lg:h-12 flex items-center justify-center bg-slate-100 hover:bg-slate-200 rounded-xl lg:rounded-2xl transition-all group"
        title="UI Customizer"
      >
        <Settings className={cn("w-5 h-5 text-slate-500 group-hover:text-primary transition-all", isOpen && "rotate-90")} />
      </button>
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="absolute right-0 mt-3 w-72 bg-white/90 backdrop-blur-xl border border-slate-200 rounded-2xl shadow-2xl z-50 overflow-hidden"
            >
              <div className="p-4 bg-slate-50/50 border-b border-slate-100 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Palette className="w-4 h-4 text-primary" />
                  <span className="text-sm font-black text-slate-800 uppercase tracking-wider">UI Preferences</span>
                </div>
                <button 
                  onClick={() => setIsOpen(false)}
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <span className={cn(
                            ...
                            theme.id === t.id ? "text-primary" : "text-slate-600"
                          )}>
                            {t.name}
                          </span>
                        </div>
