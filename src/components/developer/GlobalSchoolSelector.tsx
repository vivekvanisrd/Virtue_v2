  ...
  return (
    <div className="relative z-50">
      ...
      <button 
        ...
        className={cn(
          "flex items-center gap-3 px-4 py-2 bg-white border rounded-2xl transition-all active:scale-95 hover:bg-muted shadow-sm hover:shadow-md",
          currentSchoolId ? "border-border" : "border-border"
        )}
      >
        <div className={cn(
          "w-8 h-8 rounded-xl flex items-center justify-center transition-colors",
          currentSchoolId ? "bg-blue-50 text-blue-600" : "bg-muted text-foreground opacity-40"
        )}>
          <Globe className={cn("w-4 h-4", isPending && "animate-spin")} />
        </div>
        
        <div className="text-left hidden md:block">
           <div className="text-[10px] font-black text-foreground opacity-40 uppercase tracking-widest">
             {currentSchoolId ? "Active Context" : "Global Registry"}
           </div>
           <div className="text-xs font-black text-foreground max-w-[150px] truncate uppercase tracking-tighter">
             {currentSchool?.name || "All Schools"}
           </div>
        </div>

        <ChevronDown className={cn("w-4 h-4 text-foreground opacity-30 transition-transform", isOpen && "rotate-180")} />
      </button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className="absolute top-full mt-3 right-0 w-[300px] bg-background border border-border rounded-[2rem] shadow-2xl backdrop-blur-xl p-2 overflow-hidden"
          >
            {/* Search bar */}
            <div className="relative mb-2">
               <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-foreground opacity-30" />
               <input 
                 autoFocus
                 type="text"
                 ...
                 className="w-full bg-muted border-none rounded-xl pl-10 pr-4 py-3 text-[11px] text-foreground placeholder-foreground opacity-30 focus:ring-1 focus:ring-primary/10 transition-all font-bold"
               />
            </div>
                ...
                >
                  <div className="flex items-center gap-3">
                     ...
                     <div className="text-left overflow-hidden">
                        ...
                        <div className="text-[9px] text-foreground font-mono italic uppercase tracking-tighter">{school.id}</div>
                     </div>
                  </div>
