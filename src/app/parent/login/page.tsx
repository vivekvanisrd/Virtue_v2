...
import { requestGuardianOtpAction, verifyGuardianOtpAction } from "@/lib/actions/guardian-auth-actions";
import { Loader2, ShieldCheck, Mail, CheckCircle, AlertCircle, Sparkles } from "lucide-react";
...
export default function ParentLoginPage() {
  ...
  const handleRequestOtp = async (e: React.FormEvent) => {
    ...
    const res = await requestGuardianOtpAction(email);
    setLoading(false);
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    const res = await verifyGuardianOtpAction(email, otp);
    setLoading(false);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col justify-center items-center relative overflow-hidden px-4">
      ...
      <div className="max-w-md w-full bg-card border border-border/80 rounded-3xl p-8 shadow-2xl relative z-10">
        ...
        <p className="text-sm opacity-60 text-center mb-8 font-medium">Log in securely using your registered email address.</p>
        ...
        {step === "email" ? (
          <form onSubmit={handleRequestOtp} className="space-y-4">
            <div>
              <label className="text-[10px] font-black uppercase tracking-widest opacity-60 mb-2 block">Email Address</label>
