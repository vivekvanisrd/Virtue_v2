...
export default async function NewAdmissionsPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string; branchId?: string }> }) {
  const sp = await searchParams;
  const from = sp.from || monthStartStr();
  const to = sp.to || todayStr();

  const [result, branchesRes] = await Promise.all([
    getNewAdmissionsReport({ from, to, branchId: sp.branchId }),
    listMyBranches(),
  ]);
  const branches = branchesRes.success ? branchesRes.data : [];
  const canPickBranch = branchesRes.success && branchesRes.canPickBranch;
  const exportHref = `/simple/reports/new-admissions/export?${new URLSearchParams({ from, to, ...(sp.branchId ? { branchId: sp.branchId } : {}) }).toString()}`;

  return (
    <div style={{ display: "grid", gap: 20 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 700, margin: 0, color: "#111827" }}>New admissions report</h1>
          <p style={{ color: "#4b5563", margin: "4px 0 0" }}>Students admitted in a date range.</p>
        </div>
      ...
      {!result.success ? (
        ...
        <>
          <div style={{ display: "flex", gap: 12 }}>
            <Stat label="New admissions" value={String(result.data.count)} />
          </div>
              ...
              <tbody>
                {result.data.rows.map((r: any, i: number) => (
                  <tr key={r.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                    ...
                    <td style={tdStyle}>{r.admissionDate ? new Date(r.admissionDate).toLocaleDateString("en-IN") : "—"}</td>
                    <td style={tdStyle}>
                ...
                {result.data.rows.length === 0 && (
                  <tr><td style={tdStyle} colSpan={6}><span style={{ color: "#6b7280" }}>No admissions in this range.</span></td></tr>
                )}
              </tbody>
            </table>
          </div>
