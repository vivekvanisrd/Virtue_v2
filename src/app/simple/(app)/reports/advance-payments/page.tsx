...
export default async function AdvancePaymentsPage({ searchParams }: { searchParams: Promise<{ branchId?: string }> }) {
  const sp = await searchParams;
  const [result, branchesRes] = await Promise.all([
    getAdvancePaymentsReport({ branchId: sp.branchId }),
    listMyBranches(),
  ]);
  const branches = branchesRes.success ? branchesRes.data : [];
  const canPickBranch = branchesRes.success && branchesRes.canPickBranch;
  const exportHref = `/simple/reports/advance-payments/export${sp.branchId ? `?branchId=${sp.branchId}` : ""}`;

  return (
    <div style={{ display: "grid", gap: 20 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 700, margin: 0, color: "#111827" }}>Advance / overpaid students</h1>
          <p style={{ color: "#4b5563", margin: "4px 0 0" }}>Students who have paid more than their committed fee — a real, ongoing state, not test data.</p>
        </div>
      ...
      {!result.success ? (
        ...
        <>
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            <Stat label="Total advance held" value={money(result.data.totalAdvance)} />
              ...
              <tbody>
                {result.data.rows.map((r: any, i: number) => (
                  <tr key={r.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                    ...
                    <td style={{ ...tdStyle, textAlign: "right", color: "#15803d", fontWeight: 700 }}>{money(r.advance)}</td>
                  </tr>
                ))}
                {result.data.rows.length === 0 && (
                  <tr><td style={tdStyle} colSpan={8}><span style={{ color: "#6b7280" }}>No students are currently overpaid.</span></td></tr>
                )}
              </tbody>
            </table>
          </div>
