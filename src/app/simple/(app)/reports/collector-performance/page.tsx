...
export default async function CollectorPerformancePage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string; branchId?: string }> }) {
  const sp = await searchParams;
  const from = sp.from || monthStartStr();
  const to = sp.to || todayStr();

  const [result, branchesRes] = await Promise.all([
    getCollectorPerformanceReport({ from, to, branchId: sp.branchId }),
    listMyBranches(),
  ]);
  const branches = branchesRes.success ? branchesRes.data : [];
  const canPickBranch = branchesRes.success && branchesRes.canPickBranch;
  const exportHref = `/simple/reports/collector-performance/export?${new URLSearchParams({ from, to, ...(sp.branchId ? { branchId: sp.branchId } : {}) }).toString()}`;

  return (
    <div style={{ display: "grid", gap: 20 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 700, margin: 0, color: "#111827" }}>Collected-by report</h1>
          <p style={{ color: "#4b5563", margin: "4px 0 0" }}>Collections totalled per staff member — a cash-handling summary, not a ranking.</p>
        </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            ...
            <tbody>
              {result.data.rows.map((r: any) => (
                <tr key={r.collectedBy} style={{ borderBottom: "1px solid #f3f4f6" }}>
