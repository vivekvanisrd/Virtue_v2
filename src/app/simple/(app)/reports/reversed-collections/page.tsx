...
export default async function ReversedCollectionsPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string; branchId?: string }> }) {
  const sp = await searchParams;
  const from = sp.from || yearStartStr();
  const to = sp.to || todayStr();

  const [result, branchesRes] = await Promise.all([
    getReversedCollectionsReport({ from, to, branchId: sp.branchId }),
    listMyBranches(),
  ]);
  const branches = branchesRes.success ? branchesRes.data : [];
  const canPickBranch = branchesRes.success && branchesRes.canPickBranch;
  const exportHref = `/simple/reports/reversed-collections/export?${new URLSearchParams({ from, to, ...(sp.branchId ? { branchId: sp.branchId } : {}) }).toString()}`;

  return (
    <div style={{ display: "grid", gap: 20 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 700, margin: 0, color: "#111827" }}>Reversed collections</h1>
          <p style={{ color: "#4b5563", margin: "4px 0 0" }}>Audit trail of every voided/reversed payment — these no longer count toward any balance.</p>
        </div>
      ...
      {!result.success ? (
        ...
        <>
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            <Stat label="Total reversed" value={money(result.data.total)} />
            <Stat label="Count" value={String(result.data.count)} />
          </div>
              ...
              <tbody>
                {result.data.rows.map((r: any, i: number) => (
                  <tr key={r.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                    ...
                    <td style={{ ...tdStyle, textAlign: "right", color: "#b91c1c" }}>{money(r.amountPaid)}</td>
                    <td style={{ ...tdStyle, fontSize: 12, color: "#6b7280" }}>{r.note || "—"}</td>
                  </tr>
                ))}
                {result.data.rows.length === 0 && (
                  <tr><td style={tdStyle} colSpan={8}><span style={{ color: "#6b7280" }}>No reversed collections in this range.</span></td></tr>
                )}
              </tbody>
            </table>
          </div>
