...
import { startTripAction, verifyDriverSession } from "@/lib/actions/transport-actions-v2";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const rateLimit = await checkRedisRateLimit("driver-start-trip", payload.driverId, 3, 60, false);
    if (!rateLimit.allowed) {
      return NextResponse.json(
        { success: false, error: { code: "TOO_MANY_REQUESTS", message: "Too many start trip requests. Please wait." } },
        { status: 429 }
      );
    }
    ...
    const result = await startTripAction(body, token);
    if (!result.success) {
